...
export default function SchemaExplorer({ schema, loading = false, error = false, onSelectDatabase, activeDbName, onPreviewTable, activePreviewTable }) {
                ...
                >
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                    ...
                    <div className={`flex items-center justify-center w-6 h-6 rounded-full transition-colors ${isActive ? 'bg-white dark:bg-slate-950/50 shadow-sm' : 'group-hover:bg-white dark:group-hover:bg-slate-800'}`}>
                      <svg className={`w-3.5 h-3.5 text-slate-400 transition-transform duration-300 ${isExpanded ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M19 9l-7 7-7-7"></path></svg>
                    </div>
                  </div>
                </button>
